...
const parent_dir = path_mod.join(
  path_mod.resolve(__dirname, ".."),
  "data_collections"
);
const data_convertion = require(path_mod.join(
  parent_dir,
...
module.exports.defineTimGoneOfTwelveSections = function (tim_gone) {
  ...
  try {
    const twelveSections = require(path_mod.join(
      parent_dir,
      "tim_gone_of_twelve_sections.json"
    ));
    const zodiac = require(path_mod.join(parent_dir, "data_convertion.json"))[
      "number_to_zodiac"
    ];
    ...
    for (const position in raw_data) {
      if (raw_data.hasOwnProperty(position)) {
        const tim_gone_temp = raw_data[position];
        const key = `${tim_gone_temp}_${position}`;
        result[key] = {
          position: position,
        };
        result[key]["metaData"] = [
          metaData[tim_gone_temp] + zodiac[position],
          "character",
        ];
      }
    }
...
module.exports.getTypeOfModule = function (tim_gone, positionOf_life_point) {
  ...
  const promise = new Promise(function (resolve, reject) {
    ...
    const type_of_module_obj = require(path_mod.join(
      parent_dir,
      "type_of_module.json"
    ));
    if (type_of_module_obj) {
      try {
        const type_of_module =
          type_of_module_obj[tim_gone][`${positionOf_life_point}`.toString()];
...
/** 十年大運 **/
module.exports.settingInternvalForTenYears = function (
  type_of_module,
  type_of_people,
  positionOf_life_point
) {
  let result = new Object();
  let data = JSON.parse(
    fs.readFileSync(path_mod.join(parent_dir, "interval_for_ten_years.json"), {
      encoding: "utf-8",
    })
  );
...
/** 現行大運 **/
module.exports.settingTenYearsLiving = function (
  year_old,
  type_of_module,
  type_of_people,
  tim_gone,
  ...
  for (let steps = 0; steps < intervals.length; steps++) {
    ...
    if (
      year_old >= parseInt(temp[0], 10) &&
      year_old <= parseInt(temp[1], 10)
    ) {
      let ten_year_living = "";
      if (type_of_people == "11" || type_of_people == "00") {
        ten_year_living =
      ...
      for (const key in tim_gone) {
        ...
        if (string_arr[1] == ten_year_living) {
          //Check if the value at position of 2 is equal to the value in ten_year_living
...
/** 人/地-現行大運 **/
module.exports.adjustTenYearsLiving = function (
  FirstSec_result,
  year_old,
  type_of_module,
  type_of_people,
  ...
  let positionOf_life_point =
    FirstSec_result[Section]["life_point"]["position"];
  let intervals = data.interval[type_of_module];
...
/** 人/地盤-十年大運 **/
module.exports.adjustInternvalForTenYears = function (
  FirstSec_Result,
  type_of_module,
  type_of_people,
  positionOf_life_point,
...
module.exports.getAge = async function (birth_year, birth_month, birth_day) {
  ...
  let temp_dateObj = calendar_convertor.solarToLunar(
    date_obj.getFullYear(),
    date_obj.getMonth(),
    date_obj.getDate()
  );
  let result = [
    temp_dateObj.lunarYear,
    temp_dateObj.lunarMonth,
    temp_dateObj.lunarDay,
  ];
