...
const star_translation = require(path.join(parent_dir, "data_convertion"))[
  "star_name_translation"
];
/*
    ...
    fs.readdir(`./${dir_name}`, function (error, files) {
      ...
      files.forEach(function (file) {
        if (path.extname(file).match(".json")) {
          fs.readFile(
            `./${dir_name}/${file}`.toString(),
            function (readF_error, data) {
...
*/
module.exports.getModuleData = function (
  birth_year,
  birth_month,
  birth_day,
  birth_time,
...
module.exports.StarNameTranslation = function () {
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    if (star_translation) {
      resolve(star_translation);
    } else {
      try {
        const temp = require(path.join(parent_dir, "data_convertion"))[
          "star_name_translation"
        ];
        if (temp) {
          resolve(temp);
        } else {
          reject("Failed to import ./data_convertion.json");
        }
      } catch (error) {
        reject(error);
      }
